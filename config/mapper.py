...
try:
    from plugins.chat_voice.config.voice_config import (
        voice_config,
        azure_config,
        huggingface_config,
        vits_config,
...
try:
    if huggingface_config["open"]:
        from plugins.chat_voice.pkg.huggingface.huggingface_session_hash import (
            get_audio_wav,
        )
    ...
    if SoVitsConfig().OPEN:
        ...
        from plugins.chat_voice.pkg.sovits.sovits_text_to_speech import (
            save_sovits_wav,
            modelAnalysis,
        )
        ...
        sovits_model = modelAnalysis(
            sovits_config.MODEL_PATH,
            sovits_config.CONFIG_PATH,
            sovits_config.CLUSTER_MODEL_PATH,
            sovits_config.ENHANCE,
            sovits_config.DIFF_MODEL_PATH,
            sovits_config.DIFF_CONFIG_PATH,
            sovits_config.ONLY_DIFFUSION,
        )
    if GenShinVoice().open:
        from plugins.chat_voice.pkg.genshinvoice.genshinvoice_text_to_speech import (
            save_genshinvoice_wav,
        )

        method_mapping["mhyvoice"] = save_genshinvoice_wav
        voice_type_mapping["mhyvoice"] = "mhyvoice"
except Exception:
    logging.error("包加载错误")
    traceback.print_exc()
